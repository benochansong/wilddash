...
func save_chimera(loadout: WildDashChimeraLoadout) -> bool:
	if current_data.is_empty():
		current_data = load_data()
	current_data["chimera"] = (loadout if loadout != null else WildDashChimeraSystem.default_loadout()).to_dictionary()
	return save_current()

func load_chimera() -> WildDashChimeraLoadout:
	if current_data.is_empty():
		current_data = load_data()
	return WildDashChimeraLoadout.from_dictionary(current_data.get("chimera", {}))
...
func _validate_and_migrate(raw: Dictionary) -> Dictionary:
	...
	var chimera_value: Variant = raw.get("chimera", {})
	if typeof(chimera_value) == TYPE_DICTIONARY:
		result["chimera"] = WildDashChimeraLoadout.from_dictionary(chimera_value as Dictionary).to_dictionary()
	result["version"] = SAVE_VERSION
	return result

func _migrate_flat_settings(legacy: Dictionary, result: Dictionary) -> void:
