...
static func resolve(
	...
	var momentum_multiplier: float = float(context.get("momentum_multiplier", 1.0))
	...
	var bounded_momentum: float = clampf(momentum_multiplier, 0.70, 1.40)
	result.knockback = spec.base_knockback * mode_scale * environment_scale * result.directional_multiplier * bounded_momentum
	...
	result.stagger = spec.base_stagger * 0.35 * environment_scale * bounded_momentum
