...
static func can_racer_reach_fruit(racer: WildDashCharacterController, fruit: Node) -> bool:
	if racer == null or fruit == null:
		return false
	var access_type := get_access_type(fruit)
	if access_type == FruitAccessType.GROUND:
		return true
	...
	var required_climb := float(fruit.get_meta(&"wilddash_required_climb", 0.0))
	var required_agility := float(fruit.get_meta(&"wilddash_required_agility", 0.0))
	...
	if access_type == FruitAccessType.TREE:
		var tier := int(fruit.get_meta(&"wilddash_branch_tier", 0))
