...
## exposes shared ability intent (range/recovery/directional pressure) so bespoke
## animal kits can migrate without bypassing Stagger, Break or survival assists.
...
static func resolve(
	...
	result.knockback = spec.base_knockback * environment_scale * result.directional_multiplier
	result.stagger = spec.base_stagger * environment_scale * result.directional_multiplier
