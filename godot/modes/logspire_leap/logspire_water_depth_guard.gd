...
func _apply_final_water_depth() -> void:
	...
	for recovery_name_value: Variant in RECOVERY_WATER_Y.keys():
		...
		_set_recovery_pair_depth(world, recovery_name, water_y - BASIN_DEPTH_METERS)
	...
	print("LOGSPIRE WATER DEPTH FINAL depth=%.1fm walking_floor=false" % BASIN_DEPTH_METERS)

func _set_recovery_pair_depth(world: Node3D, recovery_name: String, floor_top_y: float) -> void:
