...
func _bind_signature_rival() -> void:
	...
	for node in get_tree().get_nodes_in_group("wilddash_ai_driver"):
		...
		print("RC7 SIGNATURE RIVAL READY racer=%s target=%.2f pressure=%.2f accel=%.2f" % [
			_racer.name, _base_target_speed, PRESSURE_SPEED_SCALE, _driver.acceleration,
		])
		return
...
func _process(_delta: float) -> void:
	...
	# Pack tactics may temporarily lower target speed for traffic. This controller
	# runs later and establishes a modest minimum pace only for the signature rival.
