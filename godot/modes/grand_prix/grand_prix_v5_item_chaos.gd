...
## Round 1 Item Distribution V3.1.
##
...
## - Avoid Dictionary -> Variant -> Vector3 casts in the station sampler. The
##   sampler now resolves the route segment with explicitly typed primitives.
##
## Stations are sampled by cumulative world distance so the opening is not packed
## with boxes while the middle of the race is empty. Long Bomb / Item Chaos live
## in the item controller and remain untouched by this pass.
...
func _spawn_item_boxes() -> void:
	...
	if _route_points.size() < 3:
		push_warning("ITEM DISTRIBUTION V3.1 route unavailable")
		return
	...
	for progress: float in ITEM_STATION_PROGRESS:
		...
		if progress >= 0.20 and progress <= 0.90:
			middle_stations += 1
	print("ITEM DISTRIBUTION V3.1 stations=%d boxes=%d progresses=%s max_gap=%.3f first_20_percent=%d middle_stations=%d wide=%d respawn=%.1fs distance_sampled=true parser_safe=true long_bomb_preserved=true" % [
	...
	if max_gap > MAX_ALLOWED_PROGRESS_GAP:
		push_warning("ITEM DISTRIBUTION V3.1 gap too large: %.3f" % max_gap)
	for i: int in range(1, ITEM_STATION_PROGRESS.size()):
		...
		if gap < MIN_EXPECTED_PROGRESS_GAP:
			push_warning("ITEM DISTRIBUTION V3.1 stations too close at %d gap=%.3f" % [i, gap])
