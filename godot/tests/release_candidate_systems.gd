...
func _ready() -> void:
	...
	_assert(SaveManager.SAVE_VERSION == 2, "save v2")
	_assert(InputMap.has_action(&"jump"), "jump action")
	_assert(InputMap.has_action(&"skill"), "skill action")
	_assert(InputMap.has_action(&"item"), "item action")
	_assert(InputMap.has_action(&"pause"), "pause action")
	_assert(_has_gamepad_event(&"jump"), "jump gamepad mapping")
	_assert(_has_gamepad_event(&"skill"), "skill gamepad mapping")
	_assert(_has_gamepad_event(&"item"), "item gamepad mapping")
	...
	var chimera := WildDashChimeraSystem.default_loadout()
	chimera.head_id = &"rabbit"
	chimera.body_id = &"elephant"
	chimera.tail_id = &"cat"
	_assert(SaveManager.save_chimera(chimera), "chimera save")
	var restored := SaveManager.load_chimera()
	_assert(restored.head_id == &"rabbit" and restored.body_id == &"elephant" and restored.tail_id == &"cat", "chimera save restore")
	...
	print("RC_SYSTEMS PASS save_v2=true keyboard=true gamepad=true pause=true settings=true chimera=true")
	get_tree().quit(0)
