...
func _init() -> void:
	...
	# V2.5 replaces ~1,135 BoxShape/edge/joint shapes with one static trimesh
	...
	var minimum_barrier_margin: float = INF
	for point_index: int in range(points.size()):
		...
		if miter_ratio > WildDashGrandPrixV2Geometry.MITER_LIMIT_RATIO + 0.001:
			...
			break
		var clearance: float = WildDashGrandPrixV2Geometry.barrier_clearance(points, widths, section_ids, point_index)
		var required: float = WildDashGrandPrixV2Geometry.barrier_required_clearance(widths, section_ids, point_index)
		minimum_barrier_margin = minf(minimum_barrier_margin, clearance - required)
		if clearance + 0.001 < required:
			failures.append("barrier intrudes inside road+shoulder at point %d" % point_index)
			break
		...
		if absf(left_distance - right_distance) > 0.01:
			failures.append("left/right barrier geometry must remain symmetric at point %d" % point_index)
			break

	for section_id: StringName in expected_ids:
		var shoulder_width: float = WildDashGrandPrixV2Geometry.shoulder_width_for_section(section_id)
		if shoulder_width < 1.0:
			failures.append("section %s needs a real shoulder before its barrier" % String(section_id))
		var profile: StringName = WildDashGrandPrixV2Geometry.barrier_profile_for_section(section_id)
		if profile == &"":
			failures.append("section %s is missing a barrier profile" % String(section_id))

	if failures.is_empty():
		print("RC9 GRAND PRIX V2.5 LAYOUT PASS sections=%d points=%d segments=%d checkpoints=%d length=%.1fm elevation=%.1f..%.1f max_y_step=%.2f road_collision_shapes=%d max_miter_ratio=%.3f min_barrier_margin=%.3f" % [
			sections.size(), points.size(), widths.size(), checkpoints.size(), length,
			elevation.x, elevation.y, max_vertical_step, expected_road_collision_shapes,
			maximum_miter_ratio, minimum_barrier_margin,
		])
		quit(0)
		return
