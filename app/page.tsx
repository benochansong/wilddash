...
export default function Home() {
  ...
  return (
    <main className={`app-shell screen-${screen} ${settings.highContrast?"high-contrast":""} ${settings.reducedMotion?"reduce-motion":""} ${settings.largeTouch?"large-touch":""}`}>
      <header className="brandbar"><button className="logo" onClick={() => setScreen("lobby")} aria-label="로비로 이동"><span>WILD</span> DASH <i>50</i></button><div className="season">SEASON 01 · SAFARI PANIC</div><div className="header-tools"><button className="sound" onClick={()=>updateSetting("sound",!settings.sound)} aria-label={settings.sound?"효과음 끄기":"효과음 켜기"}>{settings.sound?"♫":"×"}</button><button className="sound" onClick={()=>setSettingsOpen(true)} aria-label="게임 설정">⚙</button></div></header>
      {settingsOpen&&<div className="settings-backdrop" onClick={()=>setSettingsOpen(false)}><section className="settings-panel" onClick={(e)=>e.stopPropagation()}><button className="settings-close" onClick={()=>setSettingsOpen(false)}>×</button><p>GAME SETTINGS</p><h2>플레이 설정</h2><SettingRow label="효과음" note="점프·충돌·스킬 효과음" checked={settings.sound} onChange={(v)=>updateSetting("sound",v)}/><SettingRow label="화면 움직임 줄이기" note="흔들림과 반복 애니메이션 최소화" checked={settings.reducedMotion} onChange={(v)=>updateSetting("reducedMotion",v)}/><SettingRow label="고대비 모드" note="경고와 조작 요소를 더 선명하게" checked={settings.highContrast} onChange={(v)=>updateSetting("highContrast",v)}/><button className="primary compact" onClick={()=>setSettingsOpen(false)}>저장하고 돌아가기</button></section></div>}

      {screen === "lobby" && <section className="lobby">
        <div className="ticker"><span>● OFFLINE</span> 오늘의 코스: 우당탕탕 사파리 고속도로 · 인터넷 연결 없이 바로 플레이!</div>
        <div className="lobby-copy"><p className="eyebrow">50-RACER LOCAL AI RACE</p><h1>엉뚱한 동물들의<br/><em>미친 질주!</em></h1><p className="subcopy">나만의 키메라를 만들고, 장애물을 돌파하고,<br/>마지막 결승선까지 살아남으세요.</p><div className="lobby-actions"><button className="primary" onClick={beginPlay}>PLAY! <span>→</span></button><button className="secondary" onClick={() => setScreen("lab")}>🧪 키메라 연구소</button><button className="install-button" onClick={()=>setScreen("tutorial")}>🎓 훈련소</button></div><div className="profile-strip"><span>⭐ 팬 <b>{profile.fans.toLocaleString()}</b></span><span>🏆 우승 <b>{profile.wins}</b></span><span>🥇 최고 <b>{profile.best}위</b></span></div><div className="online"><b>● LOCAL</b>진행 기록은 이 PC에 자동 저장됩니다</div></div>
      ...
      {screen === "pick" && <section className="pick-page">
        <button className="back" onClick={() => setScreen("lobby")}>← 로비</button><div className="round-board"><span>NEXT ROUND · WILD GRAND PRIX</span><b>ROUND 1 · 50마리 로컬 레이스</b><h2>와일드 월드 그랑프리</h2><div><i>급커브</i><i>언덕·내리막</i><i>램프·터널</i><i>5개 테마 구간</i></div></div><p className="pick-hint">모두 같은 기본 속도로 출발 · 스킬은 5초 뒤 해제됩니다</p><div className="animal-grid">{(Object.keys(ANIMALS) as AnimalKey[]).map((key) => { const a=ANIMALS[key]; return <button key={key} className={`animal-card ${animal===key?"selected":""}`} style={{"--animal":a.color} as React.CSSProperties} onClick={() => {setAnimal(key);playTone(600,.06)}}><span className="animal-emoji">{a.emoji}</span><span className="check">✓</span><b>{a.name}</b><small>{a.label}</small><div><strong>{a.skill}</strong><span>{a.description}</span></div></button>})}</div><div className="difficulty-picker"><span>경쟁 강도</span>{(Object.keys(DIFFICULTIES) as DifficultyKey[]).map((key)=><button key={key} className={difficulty===key?"selected":""} onClick={()=>setDifficulty(key)}><b>{DIFFICULTIES[key].name}</b><small>{DIFFICULTIES[key].tag}</small></button>)}</div><div className="item-balance">{(Object.keys(ITEM_INFO) as Exclude<ItemKey,null>[]).map((key)=><span key={key}><b>{ITEM_INFO[key].emoji} {ITEM_INFO[key].level}</b><small>{ITEM_INFO[key].description}</small></span>)}</div><button className="primary deploy" onClick={startCountdown}>그랑프리 출전! <span>→</span></button><p className="control-hint"><b>조향</b> A/D · 좌우 <b>가속</b> W <b>점프</b> SPACE <b>스킬</b> E <b>아이템</b> Q</p>
      </section>}

      {screen === "countdown" && <section className="countdown-page"><div className="letterbox top"/><div className="versus"><div><Chimera {...parts} small/><b>YOU · {ANIMALS[animal].name}</b></div><span>VS</span><div className="rival"><div className="rival-animal">🦊</div><b>49 WILD ANIMALS</b></div></div><p>WILD GRAND PRIX · LOCAL AI RACE · 동일 기본 속도 · 스킬 5초 잠금</p><div className="count-number">{countdown === 0 ? "GO!" : countdown}</div><div className="letterbox bottom"/></section>}
